...
class ThreeCPGame(BaseGame):
    ...
    def reset_game(self):
        ...
        self.control_points[0].set_owner(Team.RED)     # CP-A: RED owns
        self.control_points[1].set_owner(Team.NOBODY)  # CP-B: Neutral
        self.control_points[2].set_owner(Team.BLU)     # CP-C: BLU owns
    ...
    def _update_proximity(self):
        ...
        for cp_idx in range(3):
            if self.manual_control:
                # Manual mode: use toggle states
                red = 1 if self.manual_states[cp_idx]['red'] else 0
                blu = 1 if self.manual_states[cp_idx]['blu'] else 0
                self.proximities[cp_idx].update_counts(red, blu)
            else:
                ...
                self.proximities[cp_idx].update_counts(
                    counts.get('red', 0),
                    counts.get('blu', 0)
                )
